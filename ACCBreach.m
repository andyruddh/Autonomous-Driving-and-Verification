...
safeFollowingVec = S.CheckSpec(safeFollowing)
fprintf('%d out of %d failed\n', sum(safeFollowingVec < 0), length(safeFollowingVec));

% 2
disp('Checking cruising speed when not impeded');
cruiseWhenNotImpededVec = S.CheckSpec(cruiseWhenNotImpeded);
fprintf('%d out of %d failed\n', sum(cruiseWhenNotImpededVec < 0), length(cruiseWhenNotImpededVec));

% 3
disp('Checking forward progress');
forwardProgressVec = S.CheckSpec(forwardProgress);
fprintf('%d out of %d failed\n', sum(forwardProgressVec < 0), length(forwardProgressVec));

% 4 (Duplicate spec-check. Same as 2)
% disp('Checking if cruising works');
% S.CheckSpec(cruiseWhenNotImpeded)

% 5
disp('Checking that the host stops only if required');
dontStopUnlessLeadStopsVec = S.CheckSpec(dontStopUnlessLeadStops);
fprintf('%d out of %d failed\n', sum(dontStopUnlessLeadStopsVec < 0), length(dontStopUnlessLeadStopsVec));

% 6
disp('Checking that the acceleration is negative when actual distance < dSafe');
negativeAccelerationVec = S.CheckSpec(negativeAcceleration);
fprintf('%d out of %d failed\n', sum(negativeAccelerationVec < 0), length(negativeAccelerationVec));

% 7
disp('Checking for collision');
noCollisionVec = S.CheckSpec(noCollision);
fprintf('%d out of %d failed\n', sum(noCollisionVec < 0), length(noCollisionVec));